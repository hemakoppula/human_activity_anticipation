...
private:
    ...
    int computeBodyPoseAndMotionInfo(double **data, double **pos_data, int variableNum) { 
    	...
    	jlist.push_back(HEAD_JOINT_NUM);
    	jlist.push_back(NECK_JOINT_NUM);
    	jlist.push_back(TORSO_JOINT_NUM);
    	jlist.push_back(LEFT_SHOULDER_JOINT_NUM);
    	jlist.push_back(RIGHT_SHOULDER_JOINT_NUM);
    	jlist.push_back(LEFT_HIP_JOINT_NUM);
    	jlist.push_back(LEFT_KNEE_JOINT_NUM);
    	jlist.push_back(RIGHT_HIP_JOINT_NUM);
    	jlist.push_back(RIGHT_KNEE_JOINT_NUM);
        for (int i=0; i<compareFrameNum; i++) {
    ...
    bool processData(double **data, double **pos_data) { 
        ...
        if (!frameComparable) {
           ...
           /* if (!mirrored) {
            ...
            //cout << "computed BodyPose features (" << variableNum << ")" << endl;
            //cout <<"size of features " << featureValues.size() << endl;
            ...
            //cout << "computed HandPosisiton features (" << variableNum << ")" << endl;
            //cout <<"size of features " << featureValues.size() << endl;
            // foot location
            variableNum = computeFootPosition(data,pos_data,variableNum);
            //cout << "compute FootPosition features (" << variableNum << ")" << endl;
            //cout <<"size of features " << featureValues.size() << endl;
            // hand percentile
            variableNum = computeMoreHandFeatures(variableNum);
            //cout << "compute MoreHand features (" << variableNum << ")" << endl;
            //cout <<"size of features " << featureValues.size() << endl;
            if (HEAD_AGAINST_MID_HEAP) {
                variableNum = computeHead(variableNum);
                //cout << "compute Head features (" << variableNum << ")" << endl;
                //cout <<"size of features " << featureValues.size() << endl;
